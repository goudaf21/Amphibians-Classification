...
from sklearn.metrics import average_precision_score, roc_curve, roc_auc_score, f1_score, balanced_accuracy_score
...
#     multi_clf = MultiOutputClassifier(LogisticRegression(C=0.6, penalty='l2', random_state=1))
...
# print("Best Average Precision Score: ", acc, comp)
...
# lr = MultiOutputClassifier(LogisticRegression(C=0.6, penalty='l2', random_state=1))
...
# Create empty lists for evaluation metric averaging
accuracies = []
precisions = []
f1s = []
rocs = []
for i in range(10):
    ...
    # implementing multi-output classifier with logistic regression
    multi_clf = MultiOutputClassifier(LogisticRegression(C=0.6, penalty='l2', random_state=1))
    multi_clf.fit(x_train_pca, y_train)
    y_pred = pd.DataFrame(multi_clf.predict(x_test_pca))

    # For each label class, find the accuracy and append to accuracy list
    my_accs = []
    for i in range(7):
        diff = abs(y_test.iloc[:, i].values - y_pred.iloc[:, i].values)
        my_accs.append(1 - (sum(diff)/len(diff)))

    # Calculate the metrics and append them to the appropriate list
...
# Output the evalation metrics 
print("")
print("LR_PCA Results: ")
