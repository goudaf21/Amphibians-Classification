...
def main():
    ...
    new_features = list(sorted(best_features.keys(), reverse=True))
    #print(sorted(new_features, reverse=True))

    new_features.remove('TR_1')
    new_features.remove('TR_14')
    new_features.remove('TR_12')
    new_features.remove('UR_0')
    #new_features.remove('UR_3')
    new_features.remove('FR_1')
    new_features.remove('FR_2')
    new_features.remove('FR_0')
    #new_features.remove('FR_3')
    #new_features.remove('CR_1')
    #new_features.remove('MR_0')

    #print("---After dopping variables--")
    #print(sorted(new_features, reverse=True))

    source_x = data.drop(data.columns.difference(sorted(new_features, reverse=True)[:10]), axis=1)
    ...
    for label in label_cols:
        ...
        svm = SVC(kernel="linear", C=0.2, gamma=0.05)
        ...
        #print("5-fold Cross Validation: ", cross_val_score(svm, x, y, cv=5))
    ...
    #print("")
    return averageTotal/7, rocTotal/7, precisionTotal/7, recallTotal/7

if __name__ == "__main__":
    average, roc, precision, recall = 0, 0, 0, 0
    for x in range(10):
        average += main()[0]
        roc += main()[1]
        precision += main()[2]
    ...
    #print("")
    print("Accuracy Average: %.3f " % (average/10))
    print("ROC Average: %.3f" % (roc/10))
    print("Precision Average: %.3f" % (precision/10))
    print("Recall Average: %.3f" % (recall/10))
